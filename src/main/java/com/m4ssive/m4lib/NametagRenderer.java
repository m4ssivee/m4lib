...
public class NametagRenderer {
    ...
    public Text getSuffixText(PlayerEntity player) {
        ...
        // Tüm mod suffix'lerini birleştir (combine all mod suffixes)
        ...
        for (Map.Entry<String, Function<PlayerEntity, Text>> entry : nametagSuffixProvidersByMod.entrySet()) {
            try {
                ...
                if (suffix != null && !suffix.getString().isEmpty()) {
                    ...
                    if (combined == null) {
                        combined = Text.empty().append(suffix);
                    } else {
                        combined.append(suffix);
                    }
                }
            } catch (Exception e) {
                M4Lib.LOGGER.error("[NametagRenderer] Error getting mod-specific nametag suffix for mod: {}",
                        entry.getKey(), e);
            }
        }

        // Player-specific suffix
